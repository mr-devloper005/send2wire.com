...
const archivePagination = (page: number, limit: number, total: number): SiteFeedPagination => {
...
const isRealArchivePost = (post: SitePost, category: string) => {
  const content = getContent(post)
  const status = asText((post as SitePost & { status?: string }).status).toUpperCase()
  if (status && status !== 'PUBLISHED') return false
  const type = asText(content.type).toLowerCase()
  if (type === 'comment') return false
  if (type && !['mediadistribution', 'media-distribution', 'press-release', 'news', 'press', 'media'].includes(type)) return false
  if (category && category !== 'all') {
    const postCategory = normalizeCategory(getCategory(post, ''))
    const tags = Array.isArray(post.tags) ? post.tags.map((tag) => normalizeCategory(tag)) : []
    return postCategory === category || tags.includes(category)
  }
  return true
}

async function fetchRealMediaDistributionFallback(page: number, limit: number, category: string) {
  try {
    const feed = await fetchSiteFeed<SitePost>(Math.max(100, limit * 4), { fresh: true, timeoutMs: 5000 })
    const allPosts = (feed?.posts || []).filter((post) => isRealArchivePost(post, category))
    const start = (page - 1) * limit
    return {
      posts: allPosts.slice(start, start + limit),
      pagination: feed?.pagination && allPosts.length <= limit
        ? feed.pagination
        : archivePagination(page, limit, allPosts.length),
    }
  } catch {
    return { posts: [], pagination: archivePagination(page, limit, 0) }
  }
}
...
export async function EditableTaskArchiveRoute({
  ...
  if (task === 'mediaDistribution' && !posts.length) {
    const fallback = await fetchRealMediaDistributionFallback(page, 24, category)
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#f7f4ef] text-[#111]">
        <section className="border-b border-[#07111f] bg-[#07111f] text-white">
          <div className="editable-reveal mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-12 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-16">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[#ffb4c4]">Media distribution archive</p>
            ...
            <p className="max-w-md border-l-4 border-[#e11d48] pl-5 text-sm font-bold leading-7 text-white/70">
              Press releases, brand announcements, and public campaigns organized for fast media review and distribution visibility.
            </p>
          </div>
        </section>

        <section className="border-b border-black bg-white text-[#07111f]">
          <div className="mx-auto flex max-w-[var(--editable-container)] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[#e11d48]' : 'hover:text-[#e11d48]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[#e11d48]' : 'whitespace-nowrap hover:text-[#e11d48]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-black bg-white lg:grid-cols-[1.55fr_0.85fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden border-b border-black lg:border-b-0 lg:border-r">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                ...
                <div className="mt-6 flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-[0.14em]">
                  <span className="bg-white/15 px-3 py-2">Publisher verified</span>
                  <span className="bg-white/15 px-3 py-2">Active syndication</span>
                  <span className="bg-white/15 px-3 py-2">Media reach</span>
                </div>
              </div>
            </Link>
            <div className="grid">
              <div className="border-b border-black bg-[#c92f2f] p-6 text-white">
                ...
                <p className="editorial-serif mt-3 text-3xl font-black leading-tight">Campaigns gaining distribution momentum.</p>
              </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] border-x border-black bg-[#f7f4ef] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-4 border-black pb-4">
            <h2 className="editorial-brand text-4xl font-black tracking-[-0.04em] sm:text-5xl">Latest distributions</h2>
          ...
          {remaining.length ? (
            <div className="grid border-l border-t border-black md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="editable-card-hover group border-b border-r border-black bg-white">
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-5 grid gap-2 text-[10px] font-black uppercase tracking-[0.14em] text-black/55 sm:grid-cols-2">
                      <span className="border border-black/15 px-3 py-2">Reach {index % 2 ? '76K' : '112K'}</span>
                      <span className="border border-black/15 px-3 py-2">Status Live</span>
                      <span className="border border-black/15 px-3 py-2">Publisher verified</span>
                      <span className="border border-black/15 px-3 py-2">Open campaign</span>
                    </div>
                  </div>
                </Link>
              ))}
